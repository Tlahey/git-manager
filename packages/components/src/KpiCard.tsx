import React from 'react'
import { Skeleton, Card } from '@git-manager/ui'

export interface KpiCardProps {
  icon: React.ReactNode
  label: string
  value: string | number
  sub?: string
  accent?: string
  loading?: boolean
}

export function KpiCard({ icon, label, value, sub, accent, loading }: KpiCardProps) {
  return (
    <Card
      className={`flex min-w-0 flex-1 flex-col gap-1.5 rounded-xl bg-card/60 px-4 py-3 shadow-xs backdrop-blur-xs transition-all hover:border-border/80 hover:shadow-md ${
        accent ?? ''
      }`}
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        {icon}
        <span className="text-[10px] font-medium tracking-wider uppercase">{label}</span>
      </div>
      {loading ? (
        <Skeleton className="my-1 h-6 w-12 rounded bg-muted/60" />
      ) : (
        <span className="text-2xl leading-none font-bold text-foreground">{value}</span>
      )}
      {sub && <span className="text-[10px] text-muted-foreground">{sub}</span>}
    </Card>
  )
}
