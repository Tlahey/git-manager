/** The shipped Monaco themes, written out as a table.
 *
 * Long on purpose, and deliberately one file: it is a palette per theme, one line per Monaco
 * color key, with no branching to follow — the same reason `themeTokens.ts` and
 * `gitCommandCatalog.ts` are single files. Splitting it by theme or by color family would turn
 * one lookup into several without removing a single decision from it.
 *
 * What is NOT here is the theme that follows the host app's own CSS variables at runtime — that
 * needs color conversion, live DOM reads and a memo, none of which is table data. It lives in
 * `dynamicTheme.ts`. */
