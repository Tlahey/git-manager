...
interactions et logique de formatage qui varie selon le type de fichier (texte, binaire, image,
renommage pur), avec des `if/else` empilés à remplacer par une interface `DiffRenderStrategy`.

> **Correction post-implémentation (2026-07-02)** : en relisant `DiffViewCenter.tsx` pour
> l'implémenter, l'hypothèse ne tient pas. Il n'y a qu'un seul point de branchement par type de
> contenu, un ternaire à deux branches (`diffData.isBinary ? <placeholder binaire> :
> <MonacoDiffViewer>`) — pas d'`if/else` empilés, pas de cas "image" distinct nulle part dans le
> code (`MonacoDiffViewer.tsx` non plus), et "split-view" n'est pas une stratégie de rendu
> séparée mais un simple prop (`viewMode`) passé à `MonacoDiffViewer`, qui gère les deux modes en
> interne. Introduire une interface `DiffRenderStrategy` pour un ternaire de 5 lignes serait de la
> sur-ingénierie — le principe du projet est explicite là-dessus ("three similar lines is better
> than a premature abstraction"). Pas de Strategy à extraire ici ; voir
> [14-architecture-refactor-tracking.md](14-architecture-refactor-tracking.md) actions 5.1/5.2.
>
> La vraie taille de `DiffViewCenter.tsx` (427 lignes) vient d'un header/toolbar riche (tabs,
> toggles blame/history, navigation entre changements, actions stage/discard), pas de logique de
> rendu par type de diff. Un futur découpage en sous-composant (header/toolbar séparé du contenu)
> serait une action R1 légitime si le fichier continue de grossir, mais c'est hors du scope de ce
> plan tel qu'écrit — à ajouter comme nouvelle action si besoin plutôt que de forcer le Strategy
> prévu ici.
...
3. ~~`DiffViewCenter.tsx` → extraire les stratégies de rendu par type de diff~~ — pas applicable,
   voir la correction dans la section Strategy ci-dessus.
...
- ~~Refactor `DiffViewCenter.tsx`~~ — pas applicable, voir la correction dans la section Strategy.
