...
| 6.6 | Extraire les actions (fetch/pull/push, undo/redo, stash/pop, création de branche, terminal) et l'état dérivé (loading map, notification, hasChanges/hasStashes/canUndo/canRedo) de `ActionToolbar.tsx` dans un hook `useActionToolbar` | `hooks/useActionToolbar.ts`, `components/action-toolbar/ActionToolbar.tsx` | — | ✅ |
...
terminée, 0 fichier restant appelant `lib/tauri.ts` directement hors imports de type), 6.6 ✅.
...
| 2026-07-02 | 6.6 | Audit de suivi post-migration R2 : `ActionToolbar.tsx` (327 lignes) mélangeait les mêmes catégories que `GitGraph.tsx`/`WipStagingPanel.tsx` avant elles — actions impératives (fetch/fetchAll/pull/push/undo/redo/stash/pop/créer branche/ouvrir terminal), état de chargement par action, toast de notification, valeurs dérivées (hasChanges/hasStashes/canUndo/canRedo/labels) — avec le rendu. Créé `hooks/useActionToolbar.ts`, déplacement fidèle (vérifié par un agent `architecture-reviewer` indépendant avant ouverture de la PR : aucune différence de logique, juste relocalisation + paramétrage par `t`). `ActionToolbar.tsx` réduit à 327→166 lignes. Vérifié : `pnpm typecheck` passe, revue `architecture-reviewer` PASS (R1 + R2 + hooks-rules). Pas de test manuel dans l'app — recommandé de tester fetch/pull/push, undo/redo, stash/pop, création de branche, ouverture du terminal via `pnpm dev` avant de merger. |
