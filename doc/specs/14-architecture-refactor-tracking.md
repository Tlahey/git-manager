...
| 6.5.c | **Lot 3** : `repo.api.ts` (`apiCloneRepo`, `apiInitRepo`), `git.api.ts` (`apiCreateBranch`, `apiFetchRemote`, `apiPullBranch`, `apiPushBranch`, wrappers undo-support `apiUnpinObject`/`apiObjectsExist`), `ollama.api.ts` (`apiCancelGeneration`, `apiGenerateCommitMessage`) et migrer `CloneRepoDialog.tsx`, `NewTabMenu.tsx`, `CreateBranchHereDialog.tsx`, `ActionToolbar.tsx`, `stores/undoHistory.store.ts`, `hooks/useOllamaGeneration.ts` | `api/repo.api.ts`, `api/git.api.ts`, `api/ollama.api.ts` + 6 fichiers | 6.5.a | ⬜ |

Note : `useGitHubRepos.ts`/`useGithubDeviceFlow.ts` n'importent que des **types** depuis
`lib/tauri.ts` (`import type { ... }`) — ce n'est pas une violation R2 (pas d'appel de fonction),
laissé tel quel.
...
(actions ponctuelles post-plan) : 6.1/6.2/6.3/6.4 ✅, 6.5.a ✅, 6.5.b ✅, 6.5.c ⬜.

**Prochaine étape** : **6.5.c** — dernier lot de la migration R2 : `repo.api.ts`
(`apiCloneRepo`/`apiInitRepo`), `git.api.ts` (`apiCreateBranch`/`apiFetchRemote`/`apiPullBranch`/
`apiPushBranch`/wrappers undo-support), `ollama.api.ts` (`apiCancelGeneration`/
`apiGenerateCommitMessage`), et migrer les 6 fichiers restants (`CloneRepoDialog.tsx`,
`NewTabMenu.tsx`, `CreateBranchHereDialog.tsx`, `ActionToolbar.tsx`, `undoHistory.store.ts`,
`useOllamaGeneration.ts`). Voir le détail dans la section 6.5 ci-dessus.
