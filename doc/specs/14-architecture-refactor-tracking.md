...
**Prochaine étape** : **6.5.b** — créer les wrappers de lecture manquants dans `git.api.ts` et
migrer les hooks de données (`useGitStatus`, `useGitLog`, `useBranches`, `useFileDiff`,
`useCommitDiff`, `useFileRawContents`, `useSidebarRows` + 3 composants sidebar + `StateTags.tsx`).
Puis **6.5.c** pour le reste (clone/init repo, création de branche, fetch/pull/push, Ollama,
undo-support). Voir le détail dans la section 6.5 ci-dessus.
