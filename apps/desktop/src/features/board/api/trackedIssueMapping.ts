 ...
 *
 * `currentBody` is the issue as it stands, and it is not optional out of politeness: the hidden meta
 * marker is rewritten **whole**, and a tracked card does not own all of it. The same issue can also
 * be a card on a *remote* board, whose identifier prefix and relations live in that same marker — so
 * anything this card has no opinion about is carried across rather than dropped. Leave it out and
 * renaming a tracked card silently strips the remote card's `GM-7` and every link it declared.
 */
export function bodyForTrackedCard(card: BoardCard, currentBody = ''): string {
  const { meta } = parseCardBody(currentBody)
  return composeCardBody({
    description: card.description,
    dod: card.dod,
    meta: {
      // Owned by the tracked card, and therefore replaced.
      dueDate: card.dueDate,
      blockedReason: card.blockedReason,
      linkedBranch: card.linkedBranch,
      // Owned by whoever else writes this issue, and therefore preserved.
      prefix: meta.prefix,
      links: meta.links,
    },
  })
