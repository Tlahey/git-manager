...
describe('useBoardData', () => {
  ...
  it('moving a local card onto a GitHub board creates the issue, then removes the local card', async () => {
    ...
    localBackend.getBoard.mockResolvedValue({ board: board(), cards: [card()] })
    remoteBackend.createCard.mockResolvedValue(card({ id: 'gh-9', revision: 'r-gh' }))
    remoteBackend.updateCard.mockResolvedValue(card({ id: 'gh-9' }))
    ...
    await act(async () => {
      await result.current.moveCardToBoard(card(), 'remote-board', 'todo')
    })
    ...
    // The rest of the card follows in a patch — a move that dropped the priority and the checklist
    // would be a move in name only.
    expect(remoteBackend.updateCard).toHaveBeenCalledWith(
      path,
      'remote-board',
      'gh-9',
      expect.objectContaining({ priority: 'normal', dod: '' }),
      'r-gh'
    )
    expect(localBackend.deleteCard).toHaveBeenCalledWith(path, 'b1', 'c1')
  })

  /** Same backend: the card is *moved*, keeping its id and its history, rather than recreated. */
  it('moving between two local boards hands the whole thing to the backend', async () => {
    localBackend.listBoards.mockResolvedValue([board(), board({ id: 'b2' })])
    localBackend.getBoard.mockResolvedValue({ board: board(), cards: [card()] })
    ...
    await act(async () => {
      await result.current.moveCardToBoard(card(), 'b2', 'done')
    })
  ...
  it('refuses to move a card to a board that no longer exists', async () => {
    localBackend.listBoards.mockResolvedValue([board()])
    localBackend.getBoard.mockResolvedValue({ board: board(), cards: [card()] })
    ...
    await expect(
      act(async () => result.current.moveCardToBoard(card(), 'gone', 'todo'))
    ).rejects.toThrow()
  })
  ...
  it('adding an issue to a local board creates a card tracking it', async () => {
    ...
    localBackend.getBoard.mockResolvedValue({ board: withPrefix, cards: [] })
