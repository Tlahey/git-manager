...
describe('useGraphColumnScroll', () => {
  ...
  it('starts unscrolled', () => {
    expect(renderScroll().result.current[0]).toBe(0)
  })
  ...
  describe('scrollTo (programmatic scroll)', () => {
    it('sets the offset directly, bypassing the wheel gesture', () => {
      const { result } = renderScroll()
      act(() => result.current[1](25))
      expect(result.current[0]).toBe(25)
    })

    it('clamps to the scrollable range like the wheel gesture does', () => {
      const { result } = renderScroll()
    ...
    it('is a no-op when nothing is scrollable', () => {
      const { result } = renderScroll({ ...BOUNDS, maxScrollX: 0 })
