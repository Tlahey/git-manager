...
import { Archive, ArchiveRestore, ArrowUpRight, Copy, MoreHorizontal, Trash2 } from 'lucide-react'
...
export function CardActionsMenu({
  ...
  if (!onDuplicate && !onArchive && !onUnarchive && !onConvertToIssue && !onDelete) return null
    ...
    <DropdownMenu>
      ...
      >
        ...
        {onConvertToIssue && (
          <DropdownMenuItem
            className="gap-2 text-xs"
            onSelect={onConvertToIssue}
            data-testid="card-action-convert-to-issue"
          >
            <ArrowUpRight className="h-3.5 w-3.5" />
            {t('card.dialog.convertToIssue')}
          </DropdownMenuItem>
        )}
        {onDelete && (
          <>
            {(onDuplicate || onArchive || onUnarchive || onConvertToIssue) && (
              <DropdownMenuSeparator />
            )}
            <DropdownMenuItem
