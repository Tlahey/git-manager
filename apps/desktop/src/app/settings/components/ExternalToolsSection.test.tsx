...
const TOOL_SECTIONS = [
  {
    ...
    slice: 'externalTools',
    key: 'mergeTool',
    commandKey: 'mergeToolCommand',
    label: 'Commande de fusion personnalisée',
  },
  {
    index: 1,
    slice: 'externalTools',
    key: 'diffTool',
    commandKey: 'diffToolCommand',
    label: 'Commande de comparaison personnalisée',
  },
  {
    index: 2,
    slice: 'git',
...
describe.each(TOOL_SECTIONS)(
  'ExternalToolsSection — $key',
  ({ index, slice, key, commandKey, label }) => {
    it('reflects the current selection', () => {
      useSettingsStore.setState({
        settings: {
          ...INITIAL_SETTINGS.settings,
          [slice]: { ...INITIAL_SETTINGS.settings[slice]!, [key]: 'custom' },
        },
      })
    ...
    it('shows and binds the custom-command input once "custom" is selected', async () => {
      ...
      expect(
        (useSettingsStore.getState().settings[slice] as unknown as Record<string, string>)[
          commandKey
        ]
      ).toBe('my-command')
    })
    ...
    it('switching the select to "custom" updates the store', async () => {
      ...
      expect(
        (useSettingsStore.getState().settings[slice] as unknown as Record<string, string>)[key]
      ).toBe('custom')
    })
  }
)
