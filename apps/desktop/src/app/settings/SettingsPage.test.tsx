...
vi.mock('./components/GeneralSection', () => ({ GeneralSection: () => <div data-testid="section-general" /> }))
vi.mock('./components/SshSection', () => ({ SshSection: () => <div data-testid="section-ssh" /> }))
vi.mock('./components/IntegrationSection', () => ({ IntegrationSection: () => <div data-testid="section-integrations" /> }))
vi.mock('./components/LlmSection', () => ({ LlmSection: () => <div data-testid="section-local_ai" /> }))
vi.mock('./components/ExternalToolsSection', () => ({ ExternalToolsSection: () => <div data-testid="section-external_tools" /> }))
vi.mock('./components/NotificationSection', () => ({ NotificationSection: () => <div data-testid="section-notifications" /> }))
vi.mock('./components/AppearanceSection', () => ({ AppearanceSection: () => <div data-testid="section-ui_customization" /> }))
vi.mock('./components/RewardsSection', () => ({ RewardsSection: () => <div data-testid="section-rewards" /> }))
...
describe('SettingsPage — navigation', () => {
  ...
  it('renders every section on request, including the full-bleed integrations layout', async () => {
    const user = userEvent.setup()
    render(<SettingsPage onClose={vi.fn()} />)
    for (const key of ['integrations', 'local_ai', 'external_tools', 'ui_customization', 'rewards']) {
