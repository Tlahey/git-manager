...
function renderDialog(props: Partial<{ open: boolean; onClose: () => void }> = {}) {
  ...
  return render(
    <QueryClientProvider client={client}>
      <AutosquashPreviewDialog repoPath="/repo" open={props.open ?? true} onClose={props.onClose ?? vi.fn()} />
    </QueryClientProvider>
  )
}
...
describe('AutosquashPreviewDialog', () => {
  it('shows the group summary and per-group fixups once loaded', async () => {
    ...
    renderDialog()
    await waitFor(() => expect(screen.getByText('fixup.autosquash.summary:{"count":2}')).toBeInTheDocument())
  ...
  it('disables confirm while there are no fixup groups', async () => {
    renderDialog()
    await waitFor(() => expect(screen.getByText('fixup.autosquash.summary:{"count":0}')).toBeInTheDocument())
  ...
  it('enables confirm once groups are present', async () => {
    mocked.apiAutosquashPreview.mockResolvedValue([{ baseOid: 'base1', baseSubject: 'Add feature', fixups: ['sha1'] }])
    renderDialog()
    await waitFor(() => expect(screen.getByRole('button', { name: 'fixup.autosquash.confirm' })).toBeEnabled())
  })
  ...
  it('toasts a warning (but still closes) when the run pauses on a conflict', async () => {
    ...
    await waitFor(() => expect(toastWarning).toHaveBeenCalledWith('gitTree.contextMenu.rebaseConflict'))
    expect(onClose).toHaveBeenCalledOnce()
  })
