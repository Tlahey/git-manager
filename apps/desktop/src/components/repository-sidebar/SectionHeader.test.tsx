...
describe('SectionHeader — rendering', () => {
  it('shows the title and icon', () => {
    render(<SectionHeader title="Branches" icon={<span data-testid="icon" />} isOpen={false} onToggle={vi.fn()} />)
    expect(screen.getByText('Branches')).toBeInTheDocument()
    expect(screen.getByTestId('icon')).toBeInTheDocument()
  })

  it('shows a chevron-right when closed and chevron-down when open', () => {
    const { container, rerender } = render(<SectionHeader title="Branches" icon={null} isOpen={false} onToggle={vi.fn()} />)
  ...
  it('shows the count only when provided', () => {
    const { rerender } = render(<SectionHeader title="Branches" icon={null} isOpen={false} onToggle={vi.fn()} />)
    expect(screen.queryByText('0')).not.toBeInTheDocument()

    rerender(<SectionHeader title="Branches" icon={null} isOpen={false} onToggle={vi.fn()} count={5} />)
    expect(screen.getByText('5')).toBeInTheDocument()

    rerender(<SectionHeader title="Branches" icon={null} isOpen={false} onToggle={vi.fn()} count={0} />)
  ...
  it('shows the action slot only when provided', () => {
    ...
    rerender(<SectionHeader title="Branches" icon={null} isOpen={false} onToggle={vi.fn()} action={<button data-testid="action">+</button>} />)
