...
describe('PrMetaSidebar', () => {
  it('shows a spinner while the PR is loading', () => {
    ...
    expect(screen.getByTestId('pr-meta-sidebar')).toBeInTheDocument()
  ...
  it('toggles the review composer', async () => {
    usePrDetailMock.mockReturnValue({ pr: pr(), isLoading: false })
  ...
  it('opens the reviewers editor and requests / removes reviewers', async () => {
    usePrDetailMock.mockReturnValue({ pr: pr(), isLoading: false })
    const user = userEvent.setup()
    render(<PrMetaSidebar repoPath="/repo" prNumber={7} />)
