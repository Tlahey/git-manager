...
export function GraphHeader({ columns }: GraphHeaderProps) {
  ...
  function handleResizeDown(e: React.PointerEvent, col: ResolvedColumn) {
    e.preventDefault()
    e.stopPropagation()
    ...
    function onMove(ev: PointerEvent) {
      ...
      setWidth(drag.current.key, drag.current.startWidth + delta)
    }
    function onUp() {
      drag.current = null
        ...
        {columns.map((col, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Poignée de redimensionnement (sauf colonne flex et dernière colonne) */}
              {!col.flex && !isLast && (
                <div
                  onPointerDown={(e) => handleResizeDown(e, col)}
