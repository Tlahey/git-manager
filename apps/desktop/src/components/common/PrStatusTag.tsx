...
export function PrStatusTag({ pr, onOpen }: PrStatusTagProps) {
  ...
  return (
    <button
      ...
      className={`inline-flex shrink-0 cursor-pointer items-center gap-1 rounded border px-1.5 py-0.5 text-[10px] leading-none font-medium tabular-nums transition-colors hover:brightness-110 ${pill}`}
      aria-label={label}
      title={label}
