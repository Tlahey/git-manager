...
export function extractRepoInfo(raw: any): { repo: string; repoUrl: string; fullName: string } {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function rawToMockPR(raw: any, currentUser: string): MockPR {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function rawToMockIssue(raw: any): MockIssue {
  return {
...
export async function fetchGitHubPRs(username: string, token: string): Promise<MockPR[]> {
  const data = await ghFetch(`https://api.github.com/search/issues?q=is:pr+author:${username}+is:open&per_page=50&sort=updated`, token)
  return (data.items ?? []).map((item: any) => rawToMockPR(item, username))
}

export async function fetchGitHubReviewRequestedPRs(username: string, token: string): Promise<MockPR[]> {
  const data = await ghFetch(`https://api.github.com/search/issues?q=is:pr+review-requested:${username}+is:open&per_page=50&sort=updated`, token)
...
export async function fetchGitHubIssues(username: string, token: string): Promise<MockIssue[]> {
  const data = await ghFetch(`https://api.github.com/search/issues?q=is:issue+assignee:${username}&per_page=50&sort=updated`, token)
  return (data.items ?? []).map(rawToMockIssue)
}

export async function fetchGitHubPRDetails(prApiUrl: string, token: string): Promise<any> {
  return ghFetch(prApiUrl, token)
}

export async function fetchGitHubCommitCiStatus(owner: string, repo: string, sha: string, token: string): Promise<any> {
