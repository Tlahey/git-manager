...
/**
 * The one contract both board backends implement — `BoardPage` (see `app/board/`) picks an
...
export interface BoardBackend {
  ...
  /** Closes a sprint, freezing the summary the caller computed — see `app/board/sprintStats.ts`. */
  closeBoard(
    path: string,
    boardId: string,
