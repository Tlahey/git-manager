...
export interface CardBodyMeta {
  ...
  linkedBranch?: string
  /** The card's identifier prefix. GitHub numbers the issue itself, but that number is the *issue's*
   * — a card carries its own `GM-7` so it survives moving between boards. */
  prefix?: string
