 ...
 * `commit`); kept as a single, un-typed-payload bus rather than one channel per concern because its
 * one subscriber today (achievements in `stores/game.store.ts`) wants the same simple "this git
 * action happened" signal for all of them. Add new `AppEvent` members here if/when another
 * subscriber needs one — don't add speculative events nothing listens to yet.
 *
