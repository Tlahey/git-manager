...
function account(overrides: Partial<GitHubAccount['user']> = {}): GitHubAccount {
  return {
    id: 'acc1',
    token: 'tok',
    user: { login: 'octocat', name: 'The Octocat', email: 'octo@example.com', avatarUrl: 'https://gh.example/avatar.png', ...overrides },
  }
}
...
describe('getAvatarUrl', () => {
  it('returns null when showAvatars is disabled', () => {
    getState.mockReturnValue({ settings: settings({ appearance: { showAvatars: false } as AppSettings['appearance'] }) })
  ...
  it('matches a connected GitHub account by email (case-insensitive)', () => {
    ...
    getState.mockReturnValue({ settings: settings({ github: { accounts: [acc], activeAccountId: null } }) })
  ...
  it('falls back to a Gravatar URL using an MD5 hash of the trimmed, lowercased email', () => {
    ...
    expect(getAvatarUrl(email, 'No Match')).toBe(`https://www.gravatar.com/avatar/${expectedHash}?d=identicon&s=64`)
  })
