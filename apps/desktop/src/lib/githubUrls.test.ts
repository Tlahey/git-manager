...
describe('resolveGithubUrl', () => {
  it('prefers the first known URL over guessing one', () => {
    expect(resolveGithubUrl('pull', OWNER_REPO, 7, 'https://github.enterprise.test/owner/repo/pull/7')).toBe(
      'https://github.enterprise.test/owner/repo/pull/7'
    )
  })

  it('falls through several known URLs to the first non-empty one', () => {
    expect(resolveGithubUrl('issues', OWNER_REPO, 7, null, undefined, 'https://github.com/owner/repo/issues/7')).toBe(
