...
export function usePrMergeability(
  ...
  // A GraphQL query (`mergeStateStatus`, per-check `isRequired`) on the tightest interval in the
  // app, so it is the poll that benefits most from backing off before the wall.
  const refreshInterval = useGithubPollInterval(20_000, accountId, 'graphql')
