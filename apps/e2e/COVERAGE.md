...
| Settings screen opens + **snapshot**                                                                       | settings   | keyboard (Mod+,)                                  | 📷 ✅ (general + notifications)   | 🟡 (general & notifications snapshotted; row-height persistence ✅; **ssh key generation ✅ · AI provider test-connection ✅ · rewards toggle ✅ · AI preset dropdown ✅ · GitHub OAuth device code ✅ · application icon ✅ · git identity/graph-load/scan-exclusion persistence ✅ · AI Features tab (commit instructions/pattern, daily summary toggles) ✅ · per-repo `ai_commit` override ✅**; appearance snapshot skipped on purpose, see below) |
...
**General section, AI Features tab, and the per-repo `ai_commit` override: done** (#435). General
(`settings.feature`) now also covers git identity (name/email), the graph-load fields
(`initialGraphCommits`/`lazyLoadGraphCommits`), the advanced scan exclusions/max-depth, and that the
config-file section renders — all via new `data-testid`s (`git-identity-name-input`,
`git-identity-email-input`, `scan-exclusions-tags`, `max-scan-depth-input`; `initial-graph-commits`
already had one), since none of the three existed before this. The AI Features tab
(`settings-ai.feature`, only reachable with AI on) covers commit instructions/pattern persistence
(feeding `AiCommitSection`/`commitConvention.ts`) and the daily-summary enabled/auto/save-to-repo
toggles, including that turning the whole feature off unmounts the finer controls rather than just
disabling them. `settings-repository.feature` mirrors the existing theme-override scenario for the
repository-scoped `ai_commit` category (`repo-override-commitInstructions`), confirming the global
AI Features page shows the same `overridden-badge-commitInstructions` tag theme's override does.
`upgradeRiskFeature`'s "Update to latest" path (`package-health-updates.feature`) is also now driven
end to end: after the risk report renders, the scenario closes the release-notes side panel (a real
Radix Dialog backdrop covering the row underneath — not a driver quirk) and clicks "Update to
latest" on the major-crossing package, confirms the major-version warning, and asserts the row
disappears from the outdated list.
