...
> **Tag push/delete and remote-branch delete: covered as of 2026-08-03**, via the command palette rather than the store bridge
> ([#271](https://github.com/Tlahey/git-manager/issues/271),
> [#272](https://github.com/Tlahey/git-manager/issues/272)). The bridge could not reach them: push
> and delete-local call their API straight from the native menu callback with no intermediate state
> at all, and the two remote deletions raised their dialog from `useState` inside the hook. Both
> now have palette entries (`useRefCommands`), which exist for their own sake — merging a branch or
> publishing a tag was mouse-only — and the coverage follows from that. See `tags.feature` and
> `remote-push.feature`, asserted against the bare origin with `ls-remote`.
