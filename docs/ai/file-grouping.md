...
| **UI**             | [`CommitBatchReviewPanel`](../../apps/desktop/src/features/graph/components/CommitBatchReviewPanel.tsx) via [`useCommitBatchReview`](../../apps/desktop/src/features/graph/hooks/useCommitBatchReview.ts) |
...
([`useCommitBatchReview`](../../apps/desktop/src/features/graph/hooks/useCommitBatchReview.ts)):
...
3. Any file the model **didn't place** is surfaced as an extra group, with an empty message and
   ...
   which is how it got reported. The hint has to show _before_ the tick, since prompting the tick is
...
4. Each proposal is validated with `validateCommitSubject`, recomputed on every render so it tracks
   your live edits. Non-blocking — except an accepted group with an _empty_ message, which is
...
afterwards), so the ordinary Commit button is deliberately _not_ guarded — refusing there would break
...
created one at a time. It is instead _re-runnable_: the proposals that landed are removed from the
...
At 100 files on a stock 4k window the floor _exceeds the window_: tokens drop from the start, which
...
_guarantees_ coverage — a model can drop a file from a list of summaries just as it can from a diff.
...
|                                     |                                                                                                                                                                                                                                                                       |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **N+1 calls**                       | Sequential unless the _Calls in flight_ setting says otherwise — whether concurrency helps is the provider's decision, not ours ([why](./README.md#reading-several-at-once)). Progress stays honest either way: `completed` counts files described, not requests sent |
...
| **Cancellation reaches the call**   | Stopping aborts the summary in flight, not just the next dispatch: every call goes out under a request id the run tracks, and `ai_complete` takes one ([how](./README.md#cancellation)). It costs the tokens the provider already spent, once per call in flight      |
| **Lossy**                           | Grouping quality now depends on the summaries. `area` is asked for as a _concept_, never a directory, since it is the key the reduce step groups on                                                                                                                   |
...
| Limitation                                    | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Whole files only**                          | The unit is a file, so two unrelated changes _inside one file_ cannot be split. That would need hunk-level staging, which the feature doesn't attempt                                                                                                                                                                                                                                                                                                                                        |
...
| [`useCommitBatchReview.test.ts`](../../apps/desktop/src/features/graph/hooks/useCommitBatchReview.test.ts)            | path reconciliation, leftovers, accept/reject, sequential apply, the pending-operation refusal on both generate and apply, a partial failure dropping what landed so a retry doesn't duplicate it, and an accepted-but-empty group being skipped                                                                                       |
| [`CommitBatchReviewPanel.test.tsx`](../../apps/desktop/src/features/graph/components/CommitBatchReviewPanel.test.tsx) | render/loading/error states, accept + edit callbacks, the staging-reset notice, the empty-message hint replacing the convention warning, and that the scroll pane is flex-sized rather than max-height                                                                                                                                 |
...
| [`planCommits.test.ts`](../../packages/ai/src/features/planCommits.test.ts)                                           | per-file diff slicing, a failed summary keeping its file, the progress sequence, cancellation, and style context passed through                                                                                                                                                                                                        |
...
| [`useWipCommitPanel.test.ts`](../../apps/desktop/src/features/graph/hooks/useWipCommitPanel.test.ts)                  | the same refusal on the non-AI batch mode (per group and once for the whole run), and the ordinary Commit button still committing during a merge                                                                                                                                                                                       |
