...
| **The declared window is trusted, never verified** | Nothing checks that Settings matches reality. Declare 32k while Ollama serves 4096 and the app will happily build a 30k-token prompt that the provider silently truncates from the start — the exact failure this machinery exists to avoid, re-armed by a wrong number. Raising the setting means also raising it in Ollama (Modelfile, or `OLLAMA_CONTEXT_LENGTH`) |
