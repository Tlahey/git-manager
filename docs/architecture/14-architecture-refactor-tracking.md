...
| 6.7 | Close the last backend R2 gap noted in `CLAUDE.md`: extract `services/git_branch.rs`, `git_remote.rs`, `git_stash.rs`, `git_rollback.rs`, `git_fixup.rs` out of the 5 command files that still called `git2` inline (`branch.rs`, `remote.rs`, `stash.rs`, `rollback.rs`, `fixup.rs`), all reduced to thin `#[tauri::command]` wrappers | 5 new `services/*.rs` + 5 `commands/*.rs` thinned | — | ✅ |
...
follow-up sweep prompted this backend pass in the same session).
...
| 2026-07-03 | 6.7 | Prompted by a broader "make the app clean for future development" request during the doc 16 (panels/interaction) work: audited the Rust backend for the same R2 gap `CLAUDE.md` had documented as accepted debt. Extracted `services/git_branch.rs` (list_branches/list_tags/checkout_branch/delete_branch), `git_remote.rs` (fetch/pull/push/list_remotes/add_remote/remove_remote — `pull` now shares one `&Repository` handle with the `fetch` it calls internally, instead of the original's two separate `Repository::open` calls; behavior-identical, just avoids a redundant open), `git_stash.rs` (push/pop/apply/drop/list/store/edit-message, including the `git stash store` shell-out `run_stash_store` which has no libgit2 equivalent), `git_rollback.rs` (revert_commit/reset_to_commit/get_commits_between), and `git_fixup.rs` (create_fixup_commit/list_pending_fixups/group_into_autosquash). All 5 corresponding `commands/*.rs` files reduced to thin wrappers (57/65/61/44/68 lines respectively). Deliberately preserved each function's exact original error type (`Result<T, String>` for the raw plain-string errors `rollback.rs`/`fixup.rs`/`stash.rs` already returned for OID-parse/shell failures, matching the same untyped-error convention already used in `undo.rs`) rather than converting them to `AppError` — doing so would have changed the wire format from a raw string to the `{code,message,detail}` JSON envelope, a real (if minor) behavior change outside the scope of a pure relocation. `run_autosquash` (in `fixup.rs`) intentionally stayed a command-only function since it needs `tauri::AppHandle` to shell out to `git rebase --autosquash`; only its git2-only preview/grouping logic moved to the service. Verified: `cargo build` and `cargo clippy` pass with no new warning at each of the 5 steps (confirmed via `git stash`/`cargo clippy` diff that the one pre-existing `manual_split_once` warning carried over from `branch.rs` already existed before this change); `cargo fmt --check` was also run and found pre-existing formatting debt across nearly the entire Rust codebase (`commit.rs`, `github.rs`, `log.rs`, `undo.rs`, etc. — unrelated to this session, not fixed here as it's a repo-wide reformat far outside scope) — the 5 new service files were formatted individually with `rustfmt` so at least the new code is clean. Updated `CLAUDE.md`'s architecture section, which previously listed these 5 files as the known gap, to reflect the closed state. `CLAUDE.md` in this repo also documents which domains are *intentionally* service-less (`github.rs`/`ollama.rs`/`ssh.rs`/`undo.rs` — HTTP/shell/filesystem-driven, not `git2` business logic) so a future pass doesn't mistake that for remaining debt. |
