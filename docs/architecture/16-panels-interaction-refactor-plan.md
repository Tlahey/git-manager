...
question asked: _is it currently easy to add a new tab, a new panel, or a new dialog without
...
holds one `useState<string | null>` _per dialog kind_ (`resetOid`, `revertOid`, `branchOid`), each
...
Doc 14's action 5.2 note already states: _"The file's real size (427 lines) comes from the
...
plan."_ Confirmed still true by reading [DiffViewCenter.tsx:162-385](../../apps/desktop/src/components/git-graph/DiffViewCenter.tsx#L162-L385)
...
maintained in one file). If a 4th native-menu dialog kind is added later _and_ the switch starts
...
nav _rendering_ stays bespoke per page (`SettingsPage`'s sidebar buttons vs `PullRequestsPage`'s
