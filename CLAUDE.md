...
| `packages/ai`        | The app's AI brain: `AiPresetId`/`AiProtocol` + `AI_PRESETS` registry, the connection-only `AiConnectionConfig` (persisted in Settings — no instructions/temperature there), and the **feature runtime** (`AiFeature` descriptors under `features/`, each owning its instruction + temperature + prompt-building, and — for completion features — an optional JSON `schema` for structured output; `createStreamingService`/`createCompletionService` turn one into a typed "service per feature"). Shipped features: `summaryCommitMessageFeature` (completion + JSON schema → `{ subject, body }`, one commit's message — a schema rather than a stream because grammar-constrained decoding is what keeps a reasoning model's thinking out of the commit box — driven by `composeCommitMessageFromSummaries`, which summarizes every staged file first so the subject covers the whole change rather than whichever files fitted a single prompt), `prDescriptionFeature` (streaming markdown PR body from a branch's range diff), `changeExplanationFeature` (streaming markdown explanation of one file's pending diff, read against the file's own content), `branchExplanationFeature` (streaming markdown explanation of a whole branch's range diff — same input as the PR description, written for a reader rather than a reviewer), `commitExplanationFeature` (streaming markdown explanation of one commit vs its parent, told not to paraphrase the commit's own message back), `workingExplanationFeature` (streaming markdown summary of everything uncommitted, asked to separate the unrelated things rather than merge them), `codeReviewFeature` (streaming markdown review of a diff — working tree or branch range, discriminated by its input's `scope` — the one feature *allowed* an opinion, where the explanations are explicitly forbidden one), `summaryGroupingFeature` (completion + JSON schema → `ProposedCommit[]`, splitting all working changes into a reviewable commit plan, driven by `planCommitsFromSummaries`) and `dailySummaryFeature` (completion + JSON schema → a "yesterday / today" briefing). Both commit features run a shared **map phase** (`summarizeFiles` → `fileSummaryFeature`, one call per changed file) before their single composing call — always, with no file-count threshold, so one button never means two behaviours. Add a new AI capability here, not in Rust — [docs/ai/README.md](docs/ai/README.md) documents the shared runtime and has one page per feature, including the checklist for adding one. |
