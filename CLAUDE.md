...
- No outbound network calls other than to the configured AI provider (Ollama at `http://localhost:11434` by default — see `packages/ai`'s `AI_PRESETS` for the other presets listed in Settings — Ollama, LM Studio, MLX and OpenAI all go through the `openai-compatible` protocol, Anthropic through `anthropic-messages`, and both are implemented in `services/ai_registry.rs`) and GitHub OAuth device flow ([github.rs](apps/desktop/src-tauri/src/commands/github.rs)).
