...
| `packages/ai`        | The app's AI brain: `AiPresetId`/`AiProtocol` + `AI_PRESETS` registry, the connection-only `AiConnectionConfig` (persisted in Settings — no instructions/temperature there), and the **feature runtime** (`AiFeature` descriptors under `features/`, each owning its instruction + temperature + prompt-building, and — for completion features — an optional JSON `schema` for structured output; `createStreamingService`/`createCompletionService` turn one into a typed "service per feature"). Shipped features: `commitMessageFeature` (completion + JSON schema → `{ subject, body }`, one commit's message — a schema rather than a stream because grammar-constrained decoding is what keeps a reasoning model's thinking out of the commit box; above 12 staged files `composeCommitMessageFromSummaries` takes over and writes it from per-file summaries instead, so the subject covers the whole change rather than whichever files fitted), `prDescriptionFeature` (streaming markdown PR body from a branch's range diff), `changeExplanationFeature` (streaming markdown explanation of one file's pending diff, read against the file's own content), `branchExplanationFeature` (streaming markdown explanation of a whole branch's range diff — same input as the PR description, written for a reader rather than a reviewer), `commitExplanationFeature` (streaming markdown explanation of one commit vs its parent, told not to paraphrase the commit's own message back), `workingExplanationFeature` (streaming markdown summary of everything uncommitted, asked to separate the unrelated things rather than merge them), `codeReviewFeature` (streaming markdown review of a diff — working tree or branch range, discriminated by its input's `scope` — the one feature *allowed* an opinion, where the explanations are explicitly forbidden one), `fileGroupingFeature` (completion + JSON schema → `ProposedCommit[]`, splitting all working changes into a reviewable commit plan — used on its own up to 12 changed files, above which `planCommitsFromSummaries` takes over and drives `fileSummaryFeature` once per file then `summaryGroupingFeature` once over the results, because a single prompt can no longer carry every diff) and `dailySummaryFeature` (completion + JSON schema → a "yesterday / today" briefing). Add a new AI capability here, not in Rust — [docs/ai/README.md](docs/ai/README.md) documents the shared runtime and has one page per feature, including the checklist for adding one. |
