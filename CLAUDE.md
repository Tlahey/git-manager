...
| `packages/ai`        | Provider-agnostic AI types: `AiPresetId`/`AiProtocol`, the `AI_PRESETS` registry, `AiGenerationConfig` (mirrors `commands/ai.rs`'s `AiProvider` trait split — see the frontend layering note above) |
